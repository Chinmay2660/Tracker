...
export const updateJob = async (req: AuthRequest, res: Response) => {
    try {
        ...
        if (data.columnId && data.columnId !== job.columnId.toString()) {
            ...
            const existingStageIndex = job.stageHistory.findIndex((stage) => stage.columnId.toString() === data.columnId);
            ...
            if (currentColumn && currentColumn.title.toLowerCase() === 'applied') {
                const stageHistory = [...(job.stageHistory || [])];
                const appliedStageIndex = stageHistory.findIndex((stage) => stage.columnId.toString() === job.columnId.toString());
        ...
        const updatedJob = await Job.findOneAndUpdate({ _id: id, userId: req.user._id }, updateData, { new: true });
...
export const moveJob = async (req: AuthRequest, res: Response) => {
    try {
        ...
        const existingStageIndex = job.stageHistory.findIndex((stage) => stage.columnId.toString() === columnId);
        const stageHistory = [...(job.stageHistory || [])];
        const isAppliedStage = column.title.toLowerCase() === 'applied';
        ...
        const updatedJob = await Job.findOneAndUpdate({ _id: id, userId: req.user._id }, { columnId, stageHistory, order: newOrder, interviewStages }, { new: true });
        res.json({ success: true, job: updatedJob });
    }
    catch (error: any) {
        res.status(500).json({ success: false, error: error.message });
...
export const reorderJobs = async (req: AuthRequest, res: Response) => {
    try {
        ...
        const updatePromises = jobIds.map((jobId: string, index: number) => Job.findOneAndUpdate({ _id: jobId, userId: req.user._id }, { order: index }, { new: true }));
