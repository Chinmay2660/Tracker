...
function fileDataToBuffer(raw: unknown): Buffer | undefined {
    ...
    if (typeof raw === 'object' &&
        ...
        Array.isArray((raw as {
            data?: unknown;
        }).data)) {
        const buf = Buffer.from((raw as {
            data: number[];
        }).data);
        return buf.length > 0 ? buf : undefined;
    }
...
function setResumeFileHeaders(res: Response, resume: {
    fileUrl: string;
    name: string;
}): void {
    const contentType = contentTypeForResume(resume.fileUrl);
    res.setHeader('Content-Type', contentType);
    res.setHeader('Content-Disposition', `inline; filename="${encodeURIComponent(resume.name || 'resume')}"`);
}
function sendResumeBufferFromMongo(req: Request, res: Response, buf: Buffer, resume: {
    name: string;
    fileUrl: string;
}): void {
    const contentType = contentTypeForResume(resume.fileUrl);
    ...
    try {
        ...
        if (isVercelServerless) {
            return res.status(404).json({
                ...
                hint: 'Upload this resume again from Resume Manager so the PDF is stored in the database (serverless has no persistent disk).',
            });
        }
        ...
        if (!fs.existsSync(absolutePath)) {
            return res.status(404).json({
                ...
                hint: 'If you use serverless hosting, upload this resume again so it can be stored for your account.',
            });
        }
