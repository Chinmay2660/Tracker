...
export const commonValidations = {
    string: (field: string, minLength: number = 1, maxLength: number = 1000) => body(field)
        .trim()
        .isLength({ min: minLength, max: maxLength })
        .withMessage(`${field} must be between ${minLength} and ${maxLength} characters`)
        .escape(),
    optionalString: (field: string, maxLength: number = 1000) => body(field)
        .optional()
        .trim()
    ...
    array: (field: string, maxItems: number = 50) => body(field)
        .optional()
