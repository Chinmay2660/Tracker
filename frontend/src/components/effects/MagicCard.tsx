...
function MagicCard({ children, className, gradientColor = 'rgba(45, 180, 160, 0.15)' }: MagicCardProps) {
    ...
    return (<div ref={cardRef} onMouseMove={handleMouseMove} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} className={cn('relative overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 transition-all duration-300 h-full', className)}>
      ...
      <div className="pointer-events-none absolute inset-0 rounded-xl transition-opacity duration-300" style={{
            opacity: opacity * 0.5,
