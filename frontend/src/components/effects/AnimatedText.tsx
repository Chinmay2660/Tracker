...
function AnimatedText({ text, className, variant = 'gradient' }: AnimatedTextProps) {
    if (variant === 'gradient') {
        return (<span className={cn('bg-gradient-to-r from-teal-500 via-emerald-500 to-lime-500 bg-clip-text text-transparent animate-gradient-shift bg-[length:200%_auto]', className)}>
        {text}
      </span>);
    }
    if (variant === 'shimmer') {
    ...
    if (variant === 'glow') {
        return (<span className={cn('text-teal-500 text-glow', className)}>
        {text}
      </span>);
    }
