...
function FloatingIcons() {
    return (<div className="fixed inset-0 pointer-events-none overflow-hidden">
      {icons.map(({ Icon, delay, x, y, size }, index) => (<div key={index} className="absolute animate-float opacity-20 dark:opacity-10" style={{
                left: x,
                top: y,
                animationDelay: `${delay}s`,
                animationDuration: '6s',
            }}>
          <Icon style={{ width: size, height: size }} className="text-teal-500 dark:text-teal-400"/>
        </div>))}
    </div>);
}
