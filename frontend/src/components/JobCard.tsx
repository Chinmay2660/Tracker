...
function JobCard({ job, isDragging }: JobCardProps) {
    ...
    const pointerStartPos = useRef<{
        x: number;
        y: number;
    } | null>(null);
    const didDrag = useRef(false);
    const { columns = [] } = useColumns();
    const { attributes, listeners, setNodeRef, transform, transition, isDragging: isSortableDragging, } = useSortable({ id: job._id });
    ...
    return (<>
      <Card ref={setNodeRef} style={style} {...attributes} {...listeners} onPointerDown={(e) => {
            handlePointerDown(e);
            listeners?.onPointerDown?.(e);
        }} onPointerUp={handlePointerUp} onClick={handleClick} className="p-3 sm:p-4 cursor-grab active:cursor-grabbing bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 hover:shadow-md transition-all touch-none select-none">
        <div className="space-y-3">
          ...
          {stageProgression.length > 0 && (<div className="flex flex-wrap items-center gap-1">
              {stageProgression.map((stage, idx) => (<div key={stage.stageId || idx} className="flex items-center">
                  <span className="px-1.5 py-0.5 text-[10px] font-medium rounded text-white" style={{ backgroundColor: stage.color || '#14b8a6' }}>
                    {stage.title}
                  </span>
                  {stageProgression.length > 1 && idx < stageProgression.length - 1 && (<span className="text-slate-400 dark:text-slate-300 mx-1 text-xs">→</span>)}
                </div>))}
            </div>)}
          ...
          {compensation && (<div className="text-xs text-slate-600 dark:text-slate-300 font-medium">
              {compensation}
            </div>)}

          
          {job.appliedDate && (<div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400">
              <Calendar className="w-3 h-3 flex-shrink-0"/>
              <span>Applied {format(new Date(job.appliedDate), 'MMM d, yyyy')}</span>
            </div>)}

          
          {job.jobUrl && (<a href={job.jobUrl} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="inline-flex items-center gap-1 text-xs text-teal-600 dark:text-teal-400 hover:underline">
              <ExternalLink className="w-3 h-3"/>
              View Posting
            </a>)}

          
          {job.tags.length > 0 && (<div className="flex flex-wrap gap-1.5 pt-1">
              {job.tags.slice(0, 3).map((tag, idx) => {
                const colors = getTagColor(tag);
                return (<span key={idx} className={cn('px-2 py-0.5 text-xs font-medium rounded-full', colors.bg, colors.text)}>
                    {tag}
                  </span>);
            })}
              {job.tags.length > 3 && (<span className="px-2 py-0.5 text-xs text-slate-500 dark:text-slate-400">
                  +{job.tags.length - 3}
                </span>)}
            </div>)}
        </div>
