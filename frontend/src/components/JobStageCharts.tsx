...
function JobStageCharts() {
    ...
    return (<div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-6">
        ...
        <CardContent className="px-2 sm:px-6">
          <div className="h-[220px] sm:h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 5, right: 10, left: -15, bottom: 50 }} style={{ outline: 'none' }} tabIndex={-1}>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--muted))" opacity={0.3}/>
                <XAxis dataKey="name" tick={{ fill: 'hsl(var(--muted-foreground))', fontSize: 10 }} angle={-45} textAnchor="end" height={60} interval={0} axisLine={false} tickLine={false}/>
                <YAxis tick={{ fill: 'hsl(var(--muted-foreground))', fontSize: 10 }} stroke="hsl(var(--muted))" allowDecimals={false} axisLine={false} tickLine={false} width={30}/>
                <Tooltip content={<CustomTooltip />}/>
                <Bar dataKey="count" radius={[6, 6, 0, 0]}>
                  {chartData.map((entry, index) => {
            if (!entry)
                return null;
            return (<Cell key={`cell-${index}`} fill={entry.color || FALLBACK_COLORS[index % FALLBACK_COLORS.length]}/>);
        })}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
      ...
      <Card>
        ...
        <CardContent className="px-2 sm:px-6">
          {pieData.length > 0 ? (<div className="h-[220px] sm:h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart style={{ outline: 'none' }} tabIndex={-1}>
                  <Pie data={pieData} cx="50%" cy="45%" labelLine={false} label={({ name, percent }) => {
                if (!percent || percent < 0.08)
                    return '';
                return `${(percent * 100).toFixed(0)}%`;
            }} outerRadius="70%" innerRadius="25%" fill="#8884d8" dataKey="count" paddingAngle={2}>
                    {pieData.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.color || FALLBACK_COLORS[index % FALLBACK_COLORS.length]}/>))}
                  </Pie>
                  <Tooltip content={<CustomTooltip />}/>
                  <Legend wrapperStyle={{ fontSize: '11px', paddingTop: '8px' }} formatter={(value) => <span style={{ color: 'hsl(var(--foreground))' }}>{value}</span>}/>
                </PieChart>
              </ResponsiveContainer>
            </div>) : (<div className="flex items-center justify-center h-[220px] sm:h-[300px] text-muted-foreground text-sm">
              No jobs to display
            </div>)}
        </CardContent>
      </Card>
      ...
      <Card className="lg:col-span-2">
        ...
        <CardContent>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4">
            ...
            return (<div key={item.name} className="text-center p-4 bg-muted/50 rounded-lg">
