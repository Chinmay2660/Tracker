...
export default function TagSelect({ value = [], onChange, placeholder = 'Select tags', className, }: TagSelectProps) {
    ...
    const availableTags = PREDEFINED_TAGS.filter((tag) => !value.includes(tag.label));
    useEffect(() => {
    ...
    return (<div ref={containerRef} className={cn('relative', className)}>
      ...
      <div className="flex flex-wrap gap-2 p-2 min-h-[42px] border border-input rounded-md bg-background cursor-pointer hover:border-ring transition-colors" onClick={() => setIsOpen(!isOpen)}>
        {value.length > 0 ? (value.map((tag) => {
            const color = getTagColor(tag);
            return (<span key={tag} className={cn('inline-flex items-center gap-1 px-2 py-1 rounded-md text-xs font-medium text-white', color)} onClick={(e) => e.stopPropagation()}>
                {tag}
      ...
      {isOpen && (<div className="absolute z-50 w-full mt-1 bg-popover border border-border rounded-md shadow-lg max-h-60 overflow-auto">
          {availableTags.length > 0 ? (availableTags.map((tag) => {
                ...
                return (<button key={tag.label} type="button" onClick={() => toggleTag(tag.label)} className="w-full text-left px-3 py-2 hover:bg-accent hover:text-accent-foreground transition-colors flex items-center gap-2">
