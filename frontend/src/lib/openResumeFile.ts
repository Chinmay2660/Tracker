...
export class ResumeFileError extends Error {
    constructor(message: string, public readonly code: 'SIGN_IN_REQUIRED' | 'FETCH_FAILED' | 'UNAUTHORIZED' | 'CORS_OR_NETWORK', public readonly httpStatus?: number) {
        super(message);
...
export function showResumeFetchErrorToast(err: unknown, context: 'view' | 'download'): void {
    ...
    if (err instanceof ResumeFileError && err.code === 'UNAUTHORIZED')
        return;
    ...
    const description = err instanceof ResumeFileError ? err.message : 'Something went wrong. Try again.';
...
export async function downloadResumeFile(resumeId: string, displayName: string, fileUrl: string): Promise<void> {
    try {
