...
export default function InterviewsPage() {
    ...
    return (<div className="space-y-5 sm:space-y-6">
      <PageHeader title="Interviews" description="Manage and track all your interview rounds" actions={<Button onClick={() => {
      ...
      {isLoading ? (<div className="grid gap-3">
          {[...Array(5)].map((_, i) => (<div key={i} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4 animate-pulse">
        ...
        </div>) : filteredInterviews.length === 0 ? (<Card className="border-slate-200 dark:border-slate-800">
          <CardContent className="py-12 text-center">
            ...
            {(statusFilter === 'upcoming' || statusFilter === 'all') && (<Button onClick={() => { setSelectedInterview(null); setIsFormOpen(true); }} className="bg-gradient-to-r from-teal-500 to-emerald-500 hover:from-teal-600 hover:to-emerald-600 text-white">
                <Plus className="w-4 h-4 mr-2"/>
                Add Interview
              </Button>)}
          </CardContent>
          ...
          {filteredInterviews.map((interview) => {
                ...
                const interviewDate = getInterviewDate(interview);
                const isOverdue = isInterviewOverdue(interview);
                return (<Card key={interview._id} className="border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hover:shadow-md transition-all cursor-pointer" onClick={() => {
                        setSelectedInterview(interview);
                        setIsFormOpen(true);
                    }}>
                <CardContent className="p-3 sm:p-4">
                  ...
                  <div className="flex items-start justify-between gap-2 mb-3">
                    ...
                    <div className="flex items-center gap-0.5 flex-shrink-0">
                      {(interview.status === 'pending') && (<Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-teal-600 hover:text-teal-700 hover:bg-teal-50 dark:text-teal-400 dark:hover:text-teal-300 dark:hover:bg-teal-900/20" onClick={(e) => openRescheduleDialog(interview, e)} title="Reschedule">
                  ...
                  {job?.role && (<div className="flex items-center gap-1.5 text-sm text-slate-600 dark:text-slate-400 mb-2">
                      <Building2 className="w-4 h-4 flex-shrink-0"/>
                      <span className="truncate">{job.role}</span>
                    </div>)}
                  {hr && (<div className="flex items-center gap-1.5 text-sm text-slate-600 dark:text-slate-400 mb-2">
                  ...
                  {hr?.noticePeriodLwdNote ? (<div className="text-xs text-slate-500 dark:text-slate-400 mb-2 pl-5 border-l-2 border-teal-200 dark:border-teal-800 line-clamp-3">
                  ...
                  {interview.notesMarkdown && (<div className="text-sm text-slate-500 dark:text-slate-400 line-clamp-2 mt-3 pt-3 border-t border-slate-100 dark:border-slate-800">
