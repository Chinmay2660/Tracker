...
export default function CalendarPage() {
    ...
    return (<div className="space-y-5 sm:space-y-6">
      <PageHeader title="Interview Calendar" description="View and manage your interview schedule" actions={<Button onClick={() => {
                setSelectedEvent(null);
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl p-3 sm:p-4 border border-slate-200 dark:border-slate-800 shadow-sm">
        <div className="h-[500px] sm:h-[600px] lg:h-[700px]">
          <Calendar localizer={localizer} events={events} startAccessor="start" endAccessor="end" onSelectSlot={handleSelectSlot} onSelectEvent={handleSelectEvent} selectable defaultView="month" views={['month', 'week', 'day']} drilldownView={null} onDrillDown={(date) => {
            handleSelectSlot({ start: date, end: new Date(date.getTime() + 60 * 60 * 1000) });
        }} min={new Date(1970, 0, 1, 0, 0, 0)} max={new Date(1970, 0, 1, 23, 59, 59)} scrollToTime={new Date(1970, 0, 1, 0, 0, 0)} style={{ height: '100%' }} eventPropGetter={eventStyleGetter} dayLayoutAlgorithm="no-overlap" components={{
            event: (props: any) => {
                const interview = props.event.resource as InterviewRound;
                return (<div className="text-xs px-1 truncate cursor-pointer text-white" onClick={(e) => {
                        e.stopPropagation();
      ...
      <InterviewFormDialog open={isFormOpen} onOpenChange={setIsFormOpen} interview={selectedEvent} defaultDate={selectedDate} defaultEndDate={selectedEndDate} onSuccess={() => {
            setSelectedEvent(null);
        ...
        <DialogContent onClose={() => setIsListDialogOpen(false)} className="max-w-lg max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-3 mt-4">
            ...
            const timeStr = interview.time && interview.time.trim() !== ''
                ...
                : '9:00 AM';
            return (<Card key={interview._id} className="hover:shadow-md transition-shadow border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800/50 group">
                  <CardContent className="p-4">
                    ...
                    interview.status === 'completed' ? 'bg-green-500' : 'bg-amber-500'}`}>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        {interview.status === 'pending' && (<Button variant="ghost" size="sm" className="text-teal-600 hover:text-teal-700 hover:bg-teal-50 dark:text-teal-400 dark:hover:text-teal-300 dark:hover:bg-teal-900/20" onClick={(e) => openRescheduleDialog(interview, e)} title="Reschedule">
          ...
          {listDialogDate && (<div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700">
              <Button onClick={() => {
                setSelectedEvent(null);
      ...
      <RescheduleDialog open={isRescheduleOpen} onOpenChange={setIsRescheduleOpen} interview={rescheduleInterview} job={rescheduleInterview ? getJob(rescheduleInterview.jobId) : null} onSuccess={() => {
